...
semi_wrapper = dict(
    ...
    train_cfg=dict(
        ...
        load2_from="work_dirs/dev_ssdd/phase2_pretrain_optical_sar/${percent}/${fold}/iter_11200.pth",
    ),
    test_cfg=dict(inference_on="teacher2"),
)
...
data = dict(
    ...
    train=dict(
        ...
        unsup=dict(
            type="CocoDataset",
            classes=("ship",),
            ann_file="ssdd_dev_protocol/data/"
                     "instances_train2017.${fold}@${percent}-unlabeled-dev.json",
            img_prefix="data/ssdd/JPEGImages/",
    ...
    val=dict(
        type="CocoDataset",
        classes=("ship",),
        ann_file="ssdd_dev_protocol/data/dev.json",
        img_prefix="data/ssdd/JPEGImages/",
    ),
    test=dict(
        type="CocoDataset",
...
# 开发协议版：unsup 改用排除开发图的新无标签标注；Phase2 从 dev_ssdd 新权重初始化。
work_dir = "work_dirs/dev_ssdd/phase3_dual_teacher/${percent}/${fold}"
auto_resume = False
load_from = None
resume_from = None
log_config = dict(
