...
def test_positive_weights_and_original_normalizer(branch, enabled, force, positive):
    ns = actual_model_namespace()
    # Stub only detector geometry/sampling; execute the actual weighting and loss path.
    ns.update(multi_apply=lambda *a, **k: ([torch.empty(0, 4)], [torch.empty(0, dtype=torch.long)], [None]),
